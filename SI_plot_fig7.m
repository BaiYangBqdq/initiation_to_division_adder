...
datnlist = 250 * [0.1:0.1:2];
chnlist = 150* [0.1:0.1:2];
% Abox_range_list = 0.55:0.05: 1;

DTlist = 60*ones(size(alist));
...
%     Abox_range = Abox_range_list(ir);
    ...
    for t = 1 : TT/dt
        ...
        V = V + gamma*V*dt ;
    %     Abox = Nd*datAn + DNA*Chns;
        Abox = Nd*datAn + sum(DNAcopy(1:round(Abox_range*Cperiod/dt)))/numel(DNAcopy(1:round(Abox_range*Cperiod/dt)))*Chns;
...
figure;
% subplot(1,2,1)
% plot(datnlist,smooth(meanVi,5),'linewidth',2); hold all
% xlim([0 400]);
% ylim([0 3]);
% xlabel('DnaA-boxes on datA site');
% ylabel('initiation mass');
% axis square
% 
% subplot(1,2,2)
% plot(datnlist,smooth(ii_adder_slope,5),'linewidth',2); hold all
% plot(datnlist,zeros(size(alist)),'--');
% xlim([0 400]);
% ylim([-2 1]);
% xlabel('DnaA-boxes on datA site');
...
figure;
subplot(1,2,1)
plot(chnlist+datnlist,smooth(meanVi,5),'linewidth',2); hold all
xlim([0 800]);
ylim([0 3]);
xlabel('DnaA-boxes per chromosome');
ylabel('initiation mass');
axis square

subplot(1,2,2)
plot(chnlist+datnlist,smooth(ii_adder_slope,5),'linewidth',2); hold all
plot(chnlist+datnlist,zeros(size(alist)),'--');
xlim([0 800]);
ylim([-1 1]);
xlabel('DnaA-boxes per chromosome');
ylabel('II adder slope');
...
% figure;
% subplot(1,2,1)
% plot(Abox_range_list,smooth(meanVi,5),'linewidth',2); hold all
% xlim([0.5 1]);
% ylim([0 3]);
% xlabel('DnaA-boxes on datA site');
% ylabel('initiation mass');
...
% 
% subplot(1,2,2)
% plot(Abox_range_list,smooth(ii_adder_slope,5),'linewidth',2); hold all
% plot(Abox_range_list,zeros(size(alist)),'--');
% xlim([0.5 1]);
% ylim([-1 1]);
% xlabel('DnaA-boxes on datA site');
% ylabel('II adder slope');
